...
class TodoItem {
    ...
    private static priorityStringMap: {[key in Priority]?: string} = {
        [Priority.None]: "None",
        [Priority.Low]: "Low",
    ...
    getPriorityString(): string {
        return TodoItem.priorityStringMap[this.priority] || ""
    }
    ...
    toString(): string {
        ...
        if(this.priority != Priority.None) {
            out = out + ` [${this.getPriorityString()}]`
        }
...
class TodoList {
    items: Array<TodoItem> = new Array<TodoItem>()
